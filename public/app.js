...
function setMediaButtonsAsIcons(){
  ...
  if (bp) {
    ...
    bp.setAttribute('aria-label','Fotoğraf Ekle');
    bp.title = 'Fotoğraf Ekle';
    makeIconOnly(bp);
  ...
  if (bv) {
    ...
    bv.setAttribute('aria-label','Video Ekle');
    bv.title = 'Video Ekle';
    makeIconOnly(bv);
...
function ensureMapLegend(mapInstance) {
  ...
  const Legend = L.Control.extend({
    ...
    onAdd: function() {
      ...
      div.innerHTML = `
        <div class="legend-title">Olay İkonları</div>
...
function shouldShowLegend() {
  // 1) GİRİŞ EKRANI (henüz currentUser yok)
  if (!currentUser) {
...
/* ==================== OLAY TÜRÜ FİLTRELERİNİ UYGULA (İYİ/KÖTÜ) ==================== */
...
/* ==================== OLAY EKLEYEN DOMAİN FİLTRELERİNİ UYGULA ==================== */
...
/* ==================== OLAY HARİTASINI FİLTREYE GÖRE SENKRONLA ==================== */
...
function renderEventTableRows(data) {
  ...
  // TABLO HEADER'A EXPORT BUTONU EKLE (sadece admin/supervisor için)
  ...
  try {
    ...
    events.forEach(evt => {
      ...
      if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
        console.warn('[loadExistingEvents] Geçersiz koordinat, olay:', e2.olay_id);
        return;
      }
    ...
    try { 
      ...
      console.warn('[loadExistingEvents] Lejant güncellenemedi:', e);
    }
...
function setLocateUI(running){
  ...
  if (btnUse){
    if (running) {
      btnUse.innerHTML = `<img src="/dontuseposition.svg" alt="İptal" width="20" height="20" style="opacity:0.6" />`;
      btnUse.classList.add('danger');
      btnUse.title = 'Konumu İptal Et';
    } else {
      btnUse.innerHTML = `<img src="/useposition.svg" alt="Konum" width="20" height="20" />`;
      btnUse.classList.remove('danger');
      btnUse.title = 'Konumumu Kullan';
    }
  }
  if (btnStop){ 
...
function renderMediaLists(){
        ...
        <div class="media-grid-vertical">
          ${photoUrls.map((u, idx) => (
            `<div class="media-thumb-wrapper" style="position:relative;">
              <a href="${u}" class="media-thumb" title="Foto ${idx+1}" data-open-full="img" data-in-form="true" style="display:block; position:relative;">
                <img src="${u}" alt="foto" loading="lazy" style="width:100%; height:120px; object-fit:cover; display:block; border-radius:6px;" />
              </a>
              <button class="media-remove-btn" data-remove-photo="${idx}" type="button" title="Fotoğrafı Sil" style="position:absolute; top:4px; right:4px; z-index:10;">×</button>
            </div>`
          )).join('')}
        </div>
      </div>`;
  } else {
    photoCol.innerHTML = '<div class="muted">Fotoğraf yok</div>';
  }
        ...
        <div class="media-grid-vertical">
          ${videoUrls.map((u, idx) => (
            `<div class="media-thumb-wrapper" style="position:relative;">
              ...
              <button class="media-remove-btn" data-remove-video="${idx}" type="button" title="Videoyu Sil" style="position:absolute; top:4px; right:4px; z-index:10;">×</button>
            </div>`
          )).join('')}
        </div>
      </div>`;
  } else {
    videoCol.innerHTML = '<div class="muted">Video yok</div>';
  }
  ...
  qsa('[data-remove-photo]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (confirm('Bu fotoğrafı kaldırmak istediğinize emin misiniz?')) {
  ...
  qsa('[data-remove-video]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (confirm('Bu videoyu kaldırmak istediğinize emin misiniz?')) {
        ...
        renderMediaLists();
        toast('Video kaldırıldı', 'success');
      }
    });
  });
}
...
function setSupervisorMode(mode) {
  ...
  if (submit) {
    ...
    submit.title = 'Supervisor ekleyemez';
  }
...
async function checkMe(){
  ...
  if (currentUser){ 
    ...
    // Giriş sonrası lejantı yenile
    try { ensureMapLegend(map); } catch {}
  } else { 
    markersLayer.clearLayers(); 
    
    // Çıkış sonrası lejantı yenile
    try { ensureMapLegend(map); } catch {}
  }
}
...
async function logout(){
  ...
  if (showAny) {
    try {
      ...
      console.warn('logout->public olay yüklenemedi:', e);
    }
  } else {
    try { markersLayer?.clearLayers(); } catch {}
    ensureMapLegend(map);
  ...
  try {
    ...
    console.warn('Harita yeniden yüklenemedi:', e);
  }
}
...
function ensureBackButton(){
  ...
  backBtn.onclick = () => {
    ...
    try { 
      ...
      console.warn('[BACK BTN] history.back() hatası:', e);
    }
  };
...
function setupSpeechToText() {
  ...
  const aciklamaLabel = Array.from(document.querySelectorAll('label')).find(l => l.textContent.includes('Açıklama'));
  ...
  mic.title = 'Sesle yaz';
  mic.setAttribute('aria-label', 'Sesle yaz');
  ...
  if (!SR) {
    mic.disabled = true;
    mic.title = 'Tarayıcı konuşma tanımıyor';
    return;
  }
...
(async function init(){
  (function addHeaderLocationBtn(){
    ...
    btn.innerHTML = `<img src="/useposition.svg" alt="Konum" width="20" height="20" />`;
    btn.title = 'Konumumu Kullan';
  ...
  try {
    ...
    console.warn('[MAP INIT] .env/config değerleri uygulanamadı:', e);
  }
  ...
  if (currentUser && (currentUser.role === 'admin' || currentUser.role === 'supervisor')) {
    ...
    setTimeout(() => {
      try {
        ...
        console.warn('[INIT] Admin harita hatası:', e);
      }
    }, 500);
