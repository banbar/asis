-- =========================
--  Afet Yönetimi - Şema (GÜNCEL)
--  İSTENEN DEĞİŞİKLİKLER:
--   • olay_id artık SERIAL (otomatik artar) – MAX+1 tetikleyicisi yok
--   • Olayı kimin eklediği: created_by (FK -> users.id) + created_by_display (kısa ad)
--   • Supervisor “silince” fizikî silme yerine yumuşak silme: is_active (TRUE/FALSE),
--     deactivated_by (FK -> users.id), deactivated_by_display, deactivated_at
--   • Aktif olaylar TRUE, gösterimde yalnızca aktifler listelenebilir
--   • Gerekli indeksler ve CASCADE FK’ler
--  Bu betik idempotent yazılmıştır; var olan tablolara ALTER ile eklemeler yapar.
-- =========================
